...
class RezkaProvider(BaseProvider):
    ...
    def get_details(self, anime: Anime) -> Anime:
        ...
        try:
            ...
            # Scrape description
            desc_div = soup.select_one(".b-post__description_text")
            if desc_div:
                anime._meta["description"] = desc_div.get_text(strip=True)
                
            # Scrape rating
            rating_span = soup.select_one(".b-post__info_rates .num")
            if rating_span:
                anime._meta["rating"] = f"★ {rating_span.get_text(strip=True)}"
                
            # Scrape genres
            genres = []
            for a_tag in soup.select("span[itemprop='genre']"):
                genres.append(a_tag.get_text(strip=True))
            if genres:
                anime.genres = genres
                
            # Age rating (often in tr td)
            for tr in soup.select(".b-post__info tr"):
                text = tr.get_text(strip=True)
                if "Возраст:" in text:
                    anime._meta["age_rating"] = text.replace("Возраст:", "").replace("зрителям, достигшим", "").strip()
                    
            # Comments
            comments = []
            for comment_div in soup.select("#comments-tree .text")[:3]:
                comments.append(comment_div.get_text(strip=True))
            anime._meta["comments"] = comments
            
        except Exception as exc:
            print(f"[rezka] get_details помилка: {exc}", file=sys.stderr)
